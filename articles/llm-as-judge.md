...
| ツール | 強み | 制約 |
| :--- | :--- | :--- |
| **Latitude** | 本番の失敗事例から評価を自動生成するGEPAアルゴリズムを搭載。MCC（Matthews Correlation Coefficient）を採用し、クラス不均衡下でもJudge品質を正確に測定 [^3^]。 | LGPL-3.0ライセンス（Copyleft）。プラットフォームとして発展途上。GitHub Starsは約951とコミュニティ規模は小さい [^21^]。 |
| **Braintrust** | データセット管理、プロンプト比較、統計的有意性検定を統合。GUIが極めて直感的。Free tierは1M spans/月、無制限ユーザーを許容し最も寛容 [^24^]。 | プラットフォーム本体はSaaS前提。Proプランは$249/月と中間層がなく、Free→有料の壁が高い [^14^]。Self-hosted はEnterpriseカスタム価格 [^17^]。 |
...
> 
> **補足**:
> - **E. コスト**: BraintrustはFree tierが寛容だが、Proへ移行時に$249/月の飛躍があり中間層がない [^14^]。LangfuseはOSS Self-hostedが無料で最もコスト効率が良い [^17^]。
> - **H. OSS/ロックイン**: BraintrustはSaaS前提でOSS版がないため×。LangfuseはMITライセンスで完全OSS [^17^]。
...
本番でユーザーが行った修正や、オペレーターが却下した出力を「弱いラベル」として蓄積し、そこから評価指標やルールを自動生成する。**Latitude** の GEPA アルゴリズムはこのアプローチの代表例である。MCC（Matthews Correlation Coefficient）を用いることで、クラス不均衡下でもJudgeの判定品質を正確に測定できる [^3^]。
...
評価品質の8〜9割は Judge LLM の選定に依存する。以下の指針で選定すると良い [^16^]。

| タスク複雑度 | 推奨 Judge モデル | 理由 |
| :--- | :--- | :--- |
| **簡易分類・構造検証** | Claude Haiku 4.5 / GPT-5 Nano | 高速・低コスト。分類・抽出タスクで十分な性能。 |
| **標準的な品質評価** | Claude Sonnet 4.6 / GPT-5.4 Mini | コストパフォーマンス最適。SWE-bench 79.6%級の推論力 [^16^]。 |
| **高度な推論・安全性判定** | Claude Opus 4.6 / GPT-5.4 | 細かなニュアンス判定、倫理的配慮が必要なタスク。ARC-AGI-2 68.8% [^16^]。 |
| **数学・論理検証** | o3 / o3 Pro | 複雑な数式・論理チェーンの検証。遅い・高価なためバッチ評価向き [^16^]。 |

> **重要**: 同じ回答セットを複数の Judge で評価し、スコアの乖離率を監視すること。乖離が大きいサンプルは人手監査の対象とする。
...
| **大規模バッチ。コストとフェッチの失敗原因を追いたい** | **Langfuse (Self-host)** + **DeepEval (Local)** | SaaS破産を避けつつ、多段処理のトレースとローカル評価を両立。LangfuseはMITライセンスで完全OSS [^17^]。 |
| **エージェントの複雑な多段工程を可視化したい** | **Arize Phoenix** または **Langfuse** | OpenTelemetryベースの深いトレースで、工程内の失敗を特定。 |
| **法規制が厳しく、安全性（PII等）を絶対防ぎたい** | **Giskard** + **Promptfoo** | 継続的レッドチーミング（動的多ターン攻撃・文脈認識型テスト）と脆弱性スキャンの組み合わせ [^15^][^25^]。 |
...
| **「安全性」**が不安 | Giskard で継続的レッドチーミングを実施し、脆弱性を継続的に検出する [^15^] |
| **「評価指標の生成」**が不安 | Latitude の GEPA で本番ログから評価セットを自動生成する [^3^] |
...
13. Braintrust, "Best AI evals products for self-hosted / on-prem enterprise deployments (2026)" — https://www.braintrust.dev/articles/best-self-hosted-ai-evals-tools-2026 [^17^]
14. Confident AI vs Braintrust: Head-to-Head Comparison (2026) — https://www.confident-ai.com/knowledge-base/compare/confident-ai-vs-braintrust [^14^]
15. SecurityXploded, "Best 5 AI Red Teaming Solutions for LLM Applications in 2026" — https://securityxploded.com/best-5-ai-red-teaming-solutions-for-llm-applications-in-2026.php [^15^]
16. iternal.ai, "The Definitive LLM Selection & Benchmarks Guide" (2026-03) — https://iternal.ai/llm-selection-guide [^16^]
17. Braintrust, "Best AI evals products for self-hosted / on-prem enterprise deployments" (2026-03-27) — https://www.braintrust.dev/articles/best-self-hosted-ai-evals-tools-2026 [^17^]
18. RelayPlane, "Braintrust: Cost Proxy vs Eval Platform" (2026-04-15) — https://relayplane.com/compare/braintrust [^18^]
19. Confident AI, 公式サイト — https://www.confident-ai.com/ [^19^]
20. Confident AI, "Top Confident AI Competitors and Why There's No True Alternatives" (2026-04-20) — https://www.confident-ai.com/knowledge-base/compare/top-confident-ai-competitors-and-why-theres-no-alternatives [^20^]
21. OpenAlternative, "Flowise AI vs Latitude: A Detailed Comparison" (2026-04-26) — https://openalternative.co/compare/flowise-ai/vs/latitude [^21^]
22. HackRead, "Top AI Tools for Red Teaming in 2026" (2026-02-05) — https://hackread.com/top-ai-tools-for-red-teaming-in-2026/ [^22^]
23. AIFOD, "Claude 3.5 vs GPT-4o vs Llama 4: Best LLM for Small Startups in 2026" (2026-04-13) — https://af.net/realtime/claude-3-5-vs-gpt-4o-vs-llama-4-best-llm-for-small-startups-in-2026/ [^23^]
24. Stackd, "Braintrust - Pricing, Review & Alternatives" (2026-01-18) — https://trystackd.com/tools/braintrust [^24^]
25. Giskard, "Continuous Red Teaming v2026" — https://www.giskard.ai/products/continuous-red-teaming [^25^]
...
# ファイル保存
output_path = "/mnt/agents/output/llm_as_a_judge_guide_2026.md"
