...
**結論:**
1 億行規模を対象にするなら、実務上の有力な第一候補は **Polars + GPU Engine** である。

### 1 億行規模のベンチマーク目安

1 億行規模の代表的なワークロードでは、性能差は「少し速い」では済まない水準まで広がる。

| タスク (1億行規模の代表例) | Pandas (CPU) | Polars (CPU) | cuDF / GPU | 高速化倍率 (vs Pandas) |
|----------------------------|--------------|--------------|------------|------------------------|
| GroupBy 集計 | 47.0 秒 | ~10.0 秒 | 1.2 秒 | ~39 倍 |
| Join (結合) | 267.0 秒 | 数十秒 | 1.0 秒 | ~267 倍 |

これらの値は、1 億行規模の GroupBy / Join を想定した代表例であり、スキーマやキーの分布、GPU 実行に対応した演算かどうかで変動する。
...
とくに Join は、計算量が重いだけでなく、ピークメモリも膨らみやすい。ざっくり見積もると、必要メモリは次のように表せる。
...
1 億行規模では、この一時的な膨張がそのまま VRAM 圧迫につながる。Join は「遅い処理」である以上に、**OOM を引き起こしやすい処理**でもある。
...
コストパフォーマンスを重視するなら、**L4 クラスの GPU に Polars の Lazy API を組み合わせる**構成が、性能・メモリ余裕・運用現実性のバランスを取りやすい。
