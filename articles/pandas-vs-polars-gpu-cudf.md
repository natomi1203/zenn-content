...
とくに Join は、計算量が重いだけでなく、ピークメモリも膨らみやすい。cuDF/libcudf でも `hash_join` は build 側からハッシュテーブルを構築し、その後 probe 側を照合する設計になっているため、概念的には入力表に加えてハッシュテーブルと出力ぶんのメモリが効いてくる。ざっくりした見積もりのイメージとしては、必要メモリは次のように表せる。
