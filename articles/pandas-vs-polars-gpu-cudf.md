...
CPU は DDR5 メモリを使用するため帯域幅が限られるが、GPU は GDDR6 や HBM（High Bandwidth Memory）を使用し、桁違いの帯域幅を誇る。これにより、1 億行規模のデータをメモリ上で操作する際、GPU はデータを高速に読み書きできる。
...
CPU のコアは複雑な制御処理に最適化された少数の「賢いコア」であるのに対し、GPU のコアは単純な演算を並列実行するための「多数の単純コア」である。GroupBy や Join のようなデータ並列性の高い処理では、GPU が圧倒的なスループットを発揮する。
...
GPU は SIMD（Single Instruction, Multiple Data）アーキテクチャを採用しており、同じ命令を多数のスレッドで並列実行する。条件分岐（if-else）が多いと、スレッド間で実行パスが分岐し（warp divergence）、並列性が低下する。
...
:::message alert
**⚠️ 注意:** Polars GPU Engine は 2026 年 4 月時点で **Open Beta** である。本番環境への導入時は、対応する式の範囲やフォールバック挙動を事前に検証することを強く推奨する。
:::

**結論:**
1 億行規模を対象にするなら、**Polars + GPU Engine** は実務上の有力候補の一つである。ただし、Beta である点とプラットフォーム制約は考慮に入れる必要がある。
...
- **PDS-H** は、Polars 開発チームが TPC-H を基に DataFrame ライブラリ向けに翻案した非公式ベンチマークであり、TPC-H のルールには準拠していない。したがって、TPC-H の公式結果とは直接比較できない。ベンチマークであり、TPC-H のルールには準拠していない。したがって、TPC-H の公式結果とは直接比較できない。
...
出典：[RAPIDS, "Polars GPU Engine"](https://rapids.ai/polars-gpu-engine/), [Polars, "GPU support status"](https://docs.pola.rs/user-guide/gpu/)
...
**技術的なメカニズム:**
- GPU が速い本質は、**メモリ帯域幅の差（最大 20 倍）**と**コア数の差（数百倍）**にある
- ただし、**ホスト - デバイス転送オーバーヘッド**により、小規模データでは CPU が有利
- **データタイプ**（数値 vs 文字列）で性能差が 2 倍〜8 倍と大きく変動

**失敗ケースの回避:**
- 10 万行未満のデータでは GPU は逆効果
- 文字列操作中心の処理では GPU の優位性は小さい
- 複雑な条件分岐は GPU の並列性を損なう

**実務上の判断基準:**
- 24GB VRAM（L4 クラス）が 1 億行の「現実解」
- コスト面でも GPU は ROI が見合いやすい（処理時間 1/6 ならトータルコスト 60% 減）
- ただし、**Beta 版の制約**（Linux/WSL2 のみ、未対応演算あり）は要検証

### 次のステップ

実際に GPU 導入を検討する場合は、以下の手順で進めることを推奨する。

1. **現状のボトルネック特定**: プロファイリングで Join/GroupBy の時間を計測
2. **小规模 POC**: 100 万行〜1,000 万行のサブセットで GPU 実行を検証
3. **演算カバレッジ確認**: 使用している Polars 式が GPU 対応しているか確認
4. **コスト試算**: クラウドインスタンスの時間単価と処理時間から ROI 計算
5. **本番導入判断**: Beta 版の制約を許容できるか検討

Polars GPU Engine は 2026 年 4 月時点で **Open Beta** であり、対応プラットフォームも Linux / WSL2 に限られる。全ての式が GPU 実行されるわけではなく、本番導入にはフォールバック挙動の検証が不可欠である。これらの制約を踏まえたうえでも、**Polars で身につけた宣言的なデータ処理の書き方を、そのまま GPU 時代へ拡張できる**という設計思想は、実務上の有力な方向性と言える。
