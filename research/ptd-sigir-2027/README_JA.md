...
有料実行は`kauche-app-lab`内だけ承認されています。先行`c6c5070` bundleはCustomJob `1755582940395339776`としてsynthetic L4 Vertex preflightを完了し、GCS outputに`_SUCCESS`と明示的scope marker `preflight-only:no-ptd-result`があります。このpreflightでは制限付きデータによるPTD学習もempirical readoutも行っていません。完全な65-fit executorは新しい`b3f2468` bundleに固定済みで、prospective production launch前のexact-bundle synthetic preflightは実行待ちです。

`artifact/verified/launch_bundle_evidence.json` は完成pipeline revision `b3f2468ae88d95658c8bfb6d1b13b0ecf31e8093` から2回独立生成してbyte一致した `git archive` / `gzip -n` bundle（SHA-256 `f1f510cd105b255496e04307714b024a1eb3ab7a082c0b693abaddfe7745542e`）を記録します。展開後bundleはoptional依存を含む全72テスト、引用検査、artifact検証を通過し、no-clobberで一度upload済みです。`artifact/verified/vertex_preflight_evidence.json`は、exact-bundle preflight待ちの間も先行preflightの不変identityを保持します。
...
完全な実行境界は、次の4つのfail-closed stageでmaterializeします。fit executorはlocked scheduleを消化し、`artifact/fit_execution.schema.json`準拠のinventoryを出力します。lock commandにはそのinventoryの15個すべてのfinal fixed-tree fit manifestと6個すべてのalternating-cycle manifestを、それぞれflagを繰り返して渡します。
...
`artifact/vertex_preflight_job_spec.json` はsynthetic runtime互換性検査用のlaunch不能なVertex `CustomJobSpec` templateのままです。登録済みL4 machine/container、正確なbundle hash、空output/no-clobber検査、container内の `PTD_COST_AUTHORIZED=true` gateを固定し、service accountとGCS locationはplaceholder、authorization既定値はfalseです。`make vertex-preflight-validate` は安全なlocal検証だけを行います。`scripts/validate_vertex_preflight.py` は明示的な `--authorize-cost` と具体的なservice-account/GCS値がある場合だけspecをrenderでき、`gcloud`呼び出しやjob submitは一切行いません。承認済みrender instanceは検証済みpreflightに使用し、launch固有locationを含むためsource artifact外に保持します。

`artifact/vertex_production_job_spec.json`と`scripts/run_vertex_production.sh`は独立したproduction境界を定義します。templateはcost authorizationをfalseで開始し、driver、code bundle、raw prefix、frozen teacherの正確なhashを要求します。render済みinstanceには空の一意なGCS output prefixとLab service accountも必要です。driverはtest-query materialization前にpre-test fitを完了し、scoring前にretrieval planをlockし、evidence-admission gate通過後にno-clobberでpublishしてから`_SUCCESS`を書きます。`scripts/validate_vertex_production.py`で検証・renderでき、validator自身はjob submitを行いません。
