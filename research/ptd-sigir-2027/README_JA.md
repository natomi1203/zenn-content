...
有料実行は`kauche-app-lab`内だけ承認されています。完全な65-fit executorは`b3f2468` bundleに固定され、そのexact synthetic L4 Vertex preflightをCustomJob `215211130346274816`として完了しました。GCS outputには`_SUCCESS`と明示的scope marker `preflight-only:no-ptd-result`があります。続いてhash-pinned prospective production jobをCustomJob `3111588640698925056`として投入し、最終audit時点では`PENDING`です。不変outputがadmissionを通るまでempirical readoutは昇格しません。
