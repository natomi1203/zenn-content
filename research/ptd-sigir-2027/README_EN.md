...
Paid execution is authorized only in `kauche-app-lab`. The exact `c6c5070` bundle completed a synthetic L4 Vertex preflight as CustomJob `1755582940395339776`; its GCS output includes `_SUCCESS` and the explicit scope marker `preflight-only:no-ptd-result`. No restricted-data PTD training or empirical readout occurred in that preflight. The complete 65-fit executor is now implemented and must be frozen into a new deterministic bundle before the prospective production launch.

`artifact/verified/launch_bundle_evidence.json` records a byte-identical two-run `git archive`/`gzip -n` bundle for completed pipeline revision `c6c507060a8c90e5e65d6d8478e55b1012e4c860` (SHA-256 `6d9da3e853c51511821d450e2a83a0db64ac11375bf2124011e09e4a8a1b2507`). The extracted bundle passed all 70 tests with the optional dependencies installed, citation checks, and artifact validation. That exact bundle was uploaded once with no-clobber semantics and passed the synthetic Vertex preflight; `artifact/verified/vertex_preflight_evidence.json` records the immutable job and object identities.
