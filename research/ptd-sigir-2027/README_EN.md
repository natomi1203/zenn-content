...
Paid execution is authorized only in `kauche-app-lab`. The earlier `c6c5070` bundle completed a synthetic L4 Vertex preflight as CustomJob `1755582940395339776`; its GCS output includes `_SUCCESS` and the explicit scope marker `preflight-only:no-ptd-result`. No restricted-data PTD training or empirical readout occurred in that preflight. The complete 65-fit executor is frozen in the new `b3f2468` bundle, and its exact-bundle synthetic preflight is pending before the prospective production launch.

`artifact/verified/launch_bundle_evidence.json` records a byte-identical two-run `git archive`/`gzip -n` bundle for completed pipeline revision `b3f2468ae88d95658c8bfb6d1b13b0ecf31e8093` (SHA-256 `f1f510cd105b255496e04307714b024a1eb3ab7a082c0b693abaddfe7745542e`). The extracted bundle passed all 72 tests with the optional dependencies installed, citation checks, and artifact validation, then was uploaded once with no-clobber semantics. `artifact/verified/vertex_preflight_evidence.json` retains the immutable earlier preflight while the exact-bundle preflight is pending.
