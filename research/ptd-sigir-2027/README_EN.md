...
`artifact/verified/execution_readiness_audit.json` hashes the recovered one-day TDM runner and records why it cannot be relabelled as PTD. Five of the six production components are now implemented: frozen teacher materialization; the real 5,584-item catalog and date masks; the shared two-stream HSTU-style/multiwindow-DIN trainer; exact train-only anchored reassignment; and the paired evaluation emitter. `runner/emit_evaluation.py` requires complete date--user--seed--variant metric rows, emits the canonical paired JSONL and full evaluation JSON, recomputes 10,000-resample contrasts, Holm correction, and guardrails, then self-applies the fail-closed admission gate before atomically publishing either output. Its smoke uses 8,040 synthetic rows and at least 1,000 latency observations per variant; positive fixture deltas are explicitly non-empirical. These are implementation checks, not efficacy, real-tree, or production-latency evidence. The five-day three-seed retrieval/latency runner remains before a paid launch can be proposed.
