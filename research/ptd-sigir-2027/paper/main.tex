...
Let $Y_{ui}\in\{0,1\}$ be the purchase indicator and suppose the teacher is calibrated so that $q_T(u,i)=\mathbb{E}[Y_{ui}\mid u]$. Linearity of expectation gives
\begin{equation}
m_T(u,c)=\sum_{i\in\operatorname{desc}(c)\cap C_u}q_T(u,i)
=\mathbb{E}\!\left[\sum_{i\in\operatorname{desc}(c)\cap C_u}Y_{ui}\mid u\right],
\label{eq:expected-mass}
\end{equation}
without requiring purchase indicators to be independent. Thus sum mass represents expected purchase count or additive utility recoverable from that subtree. The item scores need not sum to one across items, so this is generally not the probability of a mutually exclusive subtree event. In the special case of exactly one target and a teacher distribution normalized over all items, however, $m_T(u,c)=\Pr(i^*\in\operatorname{desc}(c)\mid u)$ and the child conditional is exactly $m_T(u,c)/m_T(u,\operatorname{parent}(c))$.

This interpretation also limits the claim. Max aggregation represents the best-one utility in a subtree, mean removes subtree cardinality from average utility, and log-sum-exp provides a smooth max-to-mass control. Sum is aligned with additive multi-item coverage, whereas max can be better for top-1 retrieval. Our primary scientific hypothesis is therefore not that sum always wins, but that appropriate subtree aggregation depends on the retrieval objective and that finite-beam pruning magnifies this objective mismatch.
...
A separate prospective mechanistic extension leaves this primary run unchanged. Its P0 comparison holds tree, seed, candidate universe, optimizer budget, and serving implementation fixed while comparing no KD/hard-path supervision, leaf-only KD, internal max, internal mean, and internal sum mass. It jointly reports best-one metrics and additive top-$K$ purchase utility/coverage, a beam-width frontier around the registered serving point, depth-wise first-prune and survival, and their paired link to final quality. P1 adds subtree-cardinality matching, within-user teacher-score shuffling, uniform and popularity-only controls, and a purchase-sparsity sweep stratified by teacher calibration. P2 adds log-sum-exp, weaker/stronger teachers, and public-data replication. Exact beam widths and stopping rules must be locked in a new immutable extension manifest before launch; no active run is stopped or reused as extension evidence. The early-pruning explanation is rejected if the aggregation difference does not shrink with wider beams or lacks the preregistered depth-survival linkage. Sum superiority is never a universal success criterion.
