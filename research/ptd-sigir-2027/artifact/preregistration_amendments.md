...
1. Reframed the prospective mechanism question: appropriate subtree aggregation may depend on the retrieval objective. Max represents best-one descendant utility; sum represents expected additive purchase count/utility; mean removes subtree cardinality; log-sum-exp is a smooth max-to-mass control. Finite beam is hypothesized to amplify these differences through irreversible early pruning. Sum is not preregistered as a universal winner.
2. Recorded the mathematical basis and its boundary. For calibrated item utilities $q_i=\mathbb{E}[Y_i\mid u]$, $\sum_{i\in L(v)}q_i=\mathbb{E}[\sum_{i\in L(v)}Y_i\mid u]$ by linearity of expectation, without item independence. It is an exact subtree probability only in the special mutually exclusive one-target case where item utilities form a normalized distribution.
3. Defined P0 as the minimum identifying experiment: no KD/hard ancestor supervision, leaf-only KD, internal max-descendant, internal mean, and internal sum-mass PTD under the same tree, seed set, split, candidate universe, optimizer budget, scorer, and serving implementation. It must report both best-one and additive top-$K$ purchase metrics, beam-width interaction around the registered serving point, depth-wise first-prune/survival, and the paired linkage from survival to final quality. Exact beam widths and stopping rules must be written to the immutable extension manifest before any P0 launch.
4. Defined P1 controls: subtree-cardinality-matched analysis; within-user teacher-score shuffling; uniform and popularity-only teachers; and a purchase-sparsity sweep jointly stratified by teacher calibration/quality. High sparsity is not assumed to help when the teacher itself degrades.
5. Defined P2 extensions: log-sum-exp, weaker/stronger teacher comparisons, and at least one public dataset with an explicit purchase/conversion proxy and a separately labelled protocol. Kauche and public results may not be pooled or used to repair one another. OTM/DTR, direct CTCVR recall distillation, and a BEAR/APAO/SmartGR-inspired prefix baseline remain required when they can be matched without changing the serving tree or budget; omissions require a pre-outcome reason.
6. Fixed falsification rules before readout. The early-pruning explanation is rejected if aggregation differences do not shrink as beam width grows or if the preregistered depth-survival linkage is absent. A universal sum-superiority claim is forbidden. A claimed additive-utility advantage must survive cardinality matching and disappear or materially weaken under teacher shuffling; contrary, null, and failed slices remain in the ledger.
7. Preserved the original eight-variant run and every currently active experiment unchanged. Existing max/log-sum-exp/top-8-mean diagnostics are context only: top-8 mean is not the registered full-mean control, and their one-day outputs cannot satisfy P0 or replace the original five-day evidence gate. Duplicate jobs are forbidden.
8. Kept the current title and all efficacy/mechanism statements pending. A mechanism-forward title may be considered only after objective-by-aggregation and beam interactions plus depth-survival linkage reproduce; a one-condition metric gain is insufficient.

Rationale: prior academic and patent work covers factorized and marginal KD, descendant summation, conversion-aware recall distillation, ranker-to-retriever transfer, and beam-aware training. The extension therefore tests a narrower, objective-conditional design principle---which aggregation matches which retrieval utility, and whether finite beam magnifies the mismatch---without retroactively changing the original confirmatory family or selecting comparisons after observing extension outcomes.
